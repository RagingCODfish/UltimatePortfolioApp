...
extension Bundle {
	func decode<T: Decodable>(
		...
		do {
			...
			fatalError("Failed to deoce \(file) from bundle due to missing \(type) value - \(context.debugDescription)")
		} catch DecodingError.dataCorrupted(_) {
			fatalError("Failed to deoce \(file) from bundle because it appears to be invalid JSON.")
		} catch {
